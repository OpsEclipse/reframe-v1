...
import type { IngestionFileStatus, IngestionStatus } from '@/lib/ingestion/types';
import { cn } from '../ui/utils';

const OldEntriesArchive = dynamic(() =>
  import('@/app/components/archive/OldEntriesArchive').then((module) => module.OldEntriesArchive),
);
...
function ImporterPopup({
  ...
  const fetchResults = useCallback(
    async (targetIngestionId: string) => {
      ...
      setResultCount(successPayload.entryCount);
      return true;
    },
    [],
  );
  ...
  const pollStatus = useCallback(
    async (targetIngestionId: string) => {
      ...
      try {
        ...
        if (successPayload.status === 'FAILED') {
          stopPolling();
        } else if (isTerminalStatus(successPayload.status)) {
          if (await fetchResults(targetIngestionId)) stopPolling();
        }
      } catch (pollError) {
        stopPolling();
